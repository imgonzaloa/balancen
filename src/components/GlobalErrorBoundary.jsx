...
function ErrorFallbackUI({ error, errorCount, onReload, onHardRefresh }) {
  const { t, lang } = useTranslation();
  const isSafeMode = errorCount >= 2;
      ...
      <div className="max-w-md w-full text-center">
        <div className="mb-8">
          ...
          <p className="text-white/60 text-sm mb-4">
            {lang === 'es' 
              ? 'Hemos registrado este error para ayudarte.'
          ...
          {isSafeMode && (
            <p className="text-amber-300 text-xs bg-amber-500/10 rounded-lg p-3 border border-amber-500/20">
              {lang === 'es' 
                ? '⚠️ Modo seguro activado: interfaz simplificada'
        ...
        <div className="space-y-3">
          ...
          >
            <Trash2 size={18} />
            {lang === 'es' ? 'Borrar caché y recargar' : 'Clear cache & reload'}
          </Button>
        </div>
