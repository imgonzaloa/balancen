import i18n from "i18next";
import { initReactI18next } from "react-i18next";

const resources = {
  en: {
    translation: {
      // Navigation
      home: "Home",
      social: "Social", 
      progress: "Progress",
      profile: "Profile",
      home_tab: "Home",
      social_tab: "Social",
      challenges_tab: "Challenges",
      progress_tab: "Progress",
      profile_tab: "Profile",
      
      // Camera flow
      point_camera_at_food: "Point camera at your food",
      initializing_camera: "Initializing camera...",
      couldnt_recognize_food: "We couldn't recognize this food",
      try_again_or_manual: "Try taking another photo or add it manually",
      retry_photo: "Retake Photo",
      add_manually: "Add Manually",
      review_and_confirm: "Review and confirm details",
      total_calories: "Total Calories",
      tap_to_edit: "Tap to edit if needed",
      photo_looks_good: "Does this look good?",
      retake: "Retake",
      use_photo: "Use Photo",
      // Analysis loader steps
      uploading_photo: "Uploading photo...",
      reading_photo: "Reading your photo...",
      identifying_foods: "Identifying foods...",
      calculating_nutrition: "Calculating nutrition...",
      finalizing: "Almost done...",
      analyzing_meal: "Analyzing meal",
      please_wait: "Please wait...",
      // Review screen
      review_meal: "Review Meal",
      confidence: "confidence",
      confirm_save: "Confirm & Save",
      
      // Home
      current_streak: "Current Streak",
      days_in_a_row: "Keep going!",
      total_fire: "Total Fire",
      today_progress: "Keep your momentum going",
      carbs: "Carbs",
      fats: "Fats",
      calories: "Calories",
      protein: "Protein",
      meals_logged_today: "Meals logged today",
      log_your_meal: "Log Your Meal",
      friends_active: "Friends active today",
      friends: "friends",
      recent_activity: "Recent Activity",
      meal: "Meal",
      meal_logged: "Meal logged!",
      meal_saved: "Meal Saved!",
      great_job: "Great job tracking",
      logged_meals: "Logged Meals",
      social_feed: "Social Feed",
      active_friends: "active friends",
      ai_coach: "AI Coach",
      ai_coach_goal_reached: "Amazing work! You hit your calorie goal. Continue this consistency to maximize results.",
      ai_coach_halfway: "Good progress. You're halfway there. Log your next meal to reach your goal.",
      ai_coach_start: "Start by logging your meals. Each photo brings you closer to understanding your nutrition.",
      todays_missions: "Today's Missions",
      last_meal: "Last Meal",
      stay_within_goal: "Stay within calorie goal",
      log_three_meals: "Log 3 meals today",
      edit_goals: "Edit Goals",
      save_changes: "Save Changes",
      goal_updated: "Goals updated",
      primary: "Primary",
      secondary: "Secondary",
      private_mode: "Private",
      friends_mode: "Friends Only",
      groups_mode: "Groups",
      
      // Paywall
      choose_plan: "Choose how far you want to go",
      free_lets_start: "Free lets you start. Premium takes you further.",
      free_plan: "Free Plan",
      premium_plan: "Premium Plan",
      includes: "Includes",
      not_included: "Not included",
      full_experience: "Full Experience",
      ai_coaching_helps: "AI Coaching helps you:",
      review_activity: "Review activity & goals",
      give_recommendations: "Give recommendations",
      adjust_habits: "Adjust habits over time",
      support_consistency: "Support consistency",
      premium_for_lasting: "Premium is designed to build lasting consistency.",
      free_for_trial: "Free is designed to try. Premium is for real results.",
      premium_pricing: "Premium Pricing",
      continue_free_limited: "Continue with free version (limited)",
      
      // Free features
      daily_checkins: "Daily check-ins",
      basic_fire_tracking: "Basic fire tracking",
      manual_tracking: "Manual tracking",
      one_group_max: "1 group max",
      basic_stats: "Basic stats",
      
      // Free limitations
      streak_capped: "Streak capped at 3 days",
      limited_fire_system: "Limited fire system",
      no_auto_goals: "No auto goals",
      no_ai_coaching: "No AI coaching",
      no_advanced_analytics: "No advanced analytics",
      no_progressive_challenges: "No progressive challenges",
      limited_social: "Limited social features",
      
      // Premium features
      unlimited_streaks: "Unlimited streaks & fire",
      three_fire_metrics: "Three fire metrics",
      auto_goal_progression: "Auto goal progression",
      ai_coaching_personalized: "AI coaching personalized",
      ai_meal_planner_feature: "AI personalized meal plans",
      ai_workout_generator_feature: "AI adaptive workout plans",
      social_feed_feature: "Social feed & posts",
      enhanced_friends_feature: "Enhanced friends system",
      advanced_analytics: "Advanced analytics insights",
      unlimited_groups: "Unlimited groups",
      social_leaderboards: "Social leaderboards",
      priority_sync: "Priority device sync",
      full_history_export: "Full history export",
      progressive_challenges: "Progressive challenges",
      unlock_social_feed: "Unlock social feed to share and connect with friends",
      unlock_friends_features: "Unlock friends features to connect with others",
      
      // Camera labels
      scan_food: "Scan Food",
      barcode: "Barcode",
      gallery: "Gallery",
      
      // Profile
      language: "Language",
      photo_updated: "Photo updated",

      // Misc
      role: "Role",
      collaborator: "Collaborator",
      estimated: "estimated",
      cal: "cal",
      invite_share_title: "Join me on the app",
      invite_share_text: "Hey! I'm inviting you to join this wellness app. Let's reach our goals together! 🔥",
      invite_shared: "Invite shared",
      link_copied: "Link copied",
      invite_error: "Failed to generate invite",
      copy_failed: "Failed to copy",
      referral_reward_text: "For every 3 friends who subscribe to Premium, get 1 free month",
      progress: "Progress",
      friends_converted: "friends converted",
      
      // Friends page
      friends_title: "Friends",
      connected: "connected",
      add_friend: "Add Friend",
      friend_email: "Friend's email",
      send_request: "Send Request",
      pending_requests: "Pending Requests",
      wants_to_connect: "wants to connect",
      my_friends: "My Friends",
      add_friends_to_share: "Add friends to share your progress",
      friend_request_sent: "Friend request sent",
      request_updated: "Request updated",
      day_streak: "day streak",
      meals: "meals",
      last_meal_label: "Last meal",
      
      // Social Feed
      share_your_progress: "Share your progress with friends",
      whats_on_your_mind: "What's on your mind?",
      no_posts_yet: "No posts yet",
      be_first_to_share: "Be the first to share your progress!",
      create_post: "Create Post",
      add_content_or_image: "Add some content or an image",
      post_created: "Post created",
      post_failed: "Failed to create post",
      just_now: "Just now",
      add_comment: "Add a comment...",
      post: "Post",
      comment_added: "Comment added",
      error_adding_comment: "Failed to add comment",
      add_image: "Add Image",
      uploading: "Uploading...",
      achievement: "Achievement",
      upload_failed: "Upload failed",
      image_uploaded: "Image uploaded",
      share_and_connect: "Share progress with friends",
      
      // AI Goals Assistant
      ai_goals_assistant: "AI Goals Assistant",
      ai_goals_desc: "Get personalized goal recommendations",
      premium_feature: "Premium Feature",
      ai_goals_premium_desc: "Unlock AI-powered personalized goal recommendations with Premium",
      current_goals: "Your Current Goals",
      daily_calories: "Daily Calories",
      not_set: "Not set",
      get_ai_recommendations: "Get AI Recommendations",
      analyzing: "Analyzing your data...",
      personalized_insight: "Personalized Insight",
      recommended_goals: "Recommended Goals",
      based_on_your_activity: "Based on your activity and goals",
      activity_goal: "Activity Goal",
      days_per_week: "days/week",
      reanalyze: "Reanalyze",
      apply_goals: "Apply Goals",
      goals_updated: "Goals updated",
      analysis_failed: "Analysis failed",
      
      // AI Coaching Tools
      ai_coaching_tools: "AI Coaching Tools",
      ai_meal_planner: "AI Meal Planner",
      personalized_meal_plans: "Personalized meal plans",
      dietary_restrictions: "Dietary Restrictions",
      dietary_placeholder: "e.g., vegan, gluten-free",
      food_preferences: "Food Preferences",
      preferences_placeholder: "e.g., loves chicken, no seafood",
      plan_duration: "Plan Duration",
      days: "days",
      generate_meal_plan: "Generate Meal Plan",
      generating: "Generating...",
      meal_plan_generated: "Meal plan generated",
      generation_failed: "Failed to generate",
      your_meal_plan: "Your Meal Plan",
      day: "Day",
      shopping_list: "Shopping List",
      tips: "Tips",
      close: "Close",
      
      ai_workout_generator: "AI Workout Generator",
      adaptive_workout_plans: "Adaptive workout plans",
      available_equipment: "Available Equipment",
      equipment_placeholder: "e.g., dumbbells, resistance bands",
      fitness_level: "Fitness Level",
      beginner: "Beginner",
      intermediate: "Intermediate",
      advanced: "Advanced",
      workout_goals: "Workout Goals",
      goals_placeholder: "e.g., build muscle, lose fat",
      session_duration: "Session Duration",
      minutes: "min",
      generate_workout: "Generate Workout",
      workout_generated: "Workout generated",
      your_workout: "Your Workout",
      warmup: "Warm-up",
      main_workout: "Main Workout",
      cooldown: "Cool-down",
      sets: "sets",
      rest: "Rest",
      weeks: "Weeks",
      
      // Groups page
      groups: "Groups",
      groups_joined: "groups joined",
      create_group: "Create Group",
      join_group: "Join Group",
      group_name_placeholder: "Group name",
      create: "Create",
      join: "Join",
      invite_code_placeholder: "Invite code",
      group_created: "Group created",
      joined_group: "Joined group",
      code_copied: "Code copied",
      no_groups_yet: "No groups yet",
      create_or_join_group: "Create or join a group to get started",
      members: "members",
      friend_request_exists: "Friend request already exists",
      group_not_found: "Group not found",
      
      // Premium page
      already_have_premium: "You already have Premium access",
      
      // UserManagement
      user_management: "User Management",
      view_manage_users: "View & manage all users",
      total_users: "Total Users",
      premium_users: "Premium",
      collaborators_count: "Collaborators",
      search_users: "Search users...",
      premium_access: "Premium Access",
      go_back: "Go Back",
      access_denied_owner: "Access denied - owner only.",
      forbidden: "403 Forbidden",
      premium_status_updated: "Premium status updated",
      
      // InviteCollaborators  
      invite_collaborators_title: "Invite Collaborators",
      grant_free_premium: "Grant free Premium access",
      new_invitation: "New Invitation",
      email_placeholder_collab: "email@example.com",
      sending: "Sending...",
      send_invitation: "Send Invitation",
      sent_invitations: "Sent Invitations",
      no_invitations_yet: "No invitations sent yet",
      pending_status: "Pending",
      accepted_status: "Accepted",
      rejected_status: "Rejected",
      invalid_email: "Invalid email",
      cannot_invite_yourself: "You cannot invite yourself",
      already_invited_email: "Already invited this email",
      invitation_sent: "Invitation sent",
      invitation_error: "Error sending invitation",
      only_owner_can_invite: "Only owner can invite collaborators",
      premium_invitation_subject: "Balancen Premium Invitation",
      premium_invitation_body: "You've been invited to join Balancen with free Premium access. Please sign up in the app using this email: {{email}}",
      
      // Home redesign
      good_morning: "Good morning",
      good_afternoon: "Good afternoon",
      good_evening: "Good evening",
      daily_intake: "Daily Intake",
      recent_meals: "Recent Meals",
      no_meals_logged: "No meals logged yet",
      start_logging: "Start logging to track your nutrition",
      streak_momentum: "Streak & Momentum",
      best_streak: "Best",
      ai_daily_insight: "AI Insight",
      unlock_ai_insights: "Unlock AI insights with Premium",
      todays_missions: "Today's Missions",
      view_all: "View all",
      kcal_short: "kcal",
      gram_short: "g",
      log_meal_button: "Log Meal",
      workout_button: "Workout",
      manual_log_description: "Search and add foods manually without taking a photo",
      premium_features: "Premium Features",
      premium_features_description: "AI Coaching, Social, Groups, Advanced Analytics & More",
      log_meal_manually: "Log Meal Manually",
      
      // Social features
      your_groups: "Your Groups",
      create_group_btn: "Create Group",
      join_group_btn: "Join Group",
      leaderboard: "Leaderboard",
      feed: "Feed",
      this_week: "This Week",
      fire_points: "Fire Points",
      no_posts_yet: "No posts yet",
      share_something: "Share your progress with the group",
      post_meal: "Share Meal",
      post_status: "Post Update",
      locked_feature: "Premium Feature",
      unlock_social: "Unlock groups, leaderboard & feed with Premium",
      upgrade_now: "Upgrade Now",
      react: "React",
      comment: "Comment",
      members_count: "{{count}} members",
      
      // Meal logging
      log_meal: "Log Meal",
      take_photo: "Take Photo",
      choose_gallery: "Choose from Gallery",
      manual_entry: "Manual Entry",
      meal_details: "Meal Details",
      
      // Empty states
      no_groups_joined: "No groups joined yet",
      create_join_group: "Create or join a group to start competing",
      
      // Social tab
      connect_compete_share: "Connect, compete, and share progress",
      meal_sharing_disabled: "Meal sharing is disabled in your privacy settings",
      no_members_yet: "No members yet",
      unknown_user: "Unknown User",
      share_invite: "Share",
      no_data_yet: "No data yet. Start logging meals!",
      loading: "Loading...",
      track_your_journey: "Track your journey and metrics",
      your_progress: "Your Progress",
      complete_analysis: "Complete analysis of your nutrition",
      momentum_score: "Momentum Score",
      never_resets: "Never resets",
      consistency_label: "Consistency",
      adherence_label: "Adherence",
      good_progress_today: "Good progress today! Keep it up.",
      invite_code: "Invite Code",
      set_status: "Set Status",
      active_challenges: "Active Challenges",
      goal: "Goal",
      ends: "Ends",
      status_placeholder: "What's on your mind?",
      characters: "characters",
      clear_status: "Clear Status",
      status_cleared: "Status cleared",
      status_too_long: "Status is too long (max 32 characters)",
      status_inappropriate: "Status contains inappropriate language",
      
      // Onboarding
      select_language: "Select Language",
      select_your_language: "Select your language",
      continue: "Continue",
      whats_main_goal: "What's your main goal?",
      choose_intensity: "Choose your intensity",
      set_calorie_limit: "Set your daily calorie limit",
      finish_setup: "Finish Setup",
      skip_for_now: "Skip for now",
      
      // Delete account
      delete_account_title: "Are you sure?",
      delete_account_message: "Are you sure you want to delete your account? This action cannot be undone.",
      delete_warning: "This action is irreversible. All your data will be deleted.",
      type_delete: "Type DELETE",
      type_delete_confirm: "Type DELETE to confirm",
      account_deleted: "Account deleted",
      delete_account_error: "Error deleting account",
      deleting: "Deleting...",
      delete: "Delete",
      
      // Camera
      camera_access_denied: "Camera access denied",
      camera_permission_denied: "Camera permission denied",
      camera_not_available: "Camera not available",
      camera_not_supported: "Camera not available. Use the gallery option.",
      camera_not_ready: "Camera not ready",
      error_capturing: "Error capturing photo",
      upload_photo: "Upload Photo",
      analyzing_meal: "Analyzing meal...",
      please_wait: "Please wait",
      analysis_failed: "Analysis failed",
      detected_items: "Detected Items",
      meal_analysis: "Meal Analysis",
      save_meal: "Save Meal",
      saving: "Saving...",
      error_saving: "Error saving meal",
      upload_photo_instead: "Upload a photo instead",
      cancel: "Cancel",
      choose_from_gallery: "Choose from Gallery",
      take_photo: "Take Photo",
      scan_food: "Scan Food",
      barcode: "Barcode",
      gallery: "Gallery",
      
      // Profile
      my_profile: "My Profile",
      edit: "Edit",
      save: "Save",
      best_streak: "Best Streak",
      total_checkins: "Total Check-ins",
      add_status: "Add a status",
      tap_to_edit: "Tap to edit",
      status_expires_24h: "Expires in 24h",
      settings: "Settings",
      settings_desc: "Language, privacy & more",
      your_goals: "Your Goals",
      main_goal: "Main Goal",
      intensity: "Intensity",
      mode: "Mode",
      with_friends: "With Friends",
      alone: "Solo",
      optional_data: "Optional Data",
      weight_kg: "Weight (kg)",
      height_cm: "Height (cm)",
      optional: "Optional",
      not_defined: "Not set",
      logout: "Logout",
      status: "Status",
      status_placeholder: "What's on your mind?",
      your_goal: "Your Goal",
      primary_goal: "Primary goal",
      daily_goal: "Daily goal",
      
      // Goals
      consistency: "Be more consistent",
      weight_loss: "Lose weight",
      healthy_habits: "Build healthy habits",
      stay_active: "Stay active",
      
      // Intensity
      easy: "Easy",
      easy_desc: "Gentle pace",
      normal: "Normal",
      normal_desc: "Balanced approach",
      challenging: "Challenging",
      challenging_desc: "Push yourself",
      
      // Settings
      language: "Language",
      select_language: "Select Language",
      privacy: "Privacy",
      data_encrypted: "Your data is encrypted",
      admin_tools: "Admin Tools",
      invite_collaborators: "Invite Collaborators",
      grant_free_premium: "Grant free premium access",
      user_management: "User Management",
      view_manage_users: "View & manage users",
      daily_calories_limit: "Daily calorie limit",
      optional_leave_empty: "Optional - leave empty if unsure",
      goals_targets: "Goals & Targets",
      set_daily_goals: "Set your daily goals",
      session_expired: "Session expired",
      please_login_again: "Please login again",
      connection_error: "Connection error",
      check_internet: "Check your internet connection",
      
      // Referral
      invite_friends: "Invite Friends",
      your_referrals: "Your referrals",
      free_month_progress: "for your next free month",
      
      // Premium differentiation
      free_limited_history: "Today's data only",
      premium_full_history: "Complete history & analytics",
      week_view: "Week View",
      month_view: "Month View",
      year_view: "Year View",
      ai_food_recognition: "AI-powered food recognition",
      adaptive_targets: "Adaptive calorie & macro targets",
      social_competition: "Social competition & teams",
      advanced_dashboards: "Advanced dashboards",
      invited_friends: "Invited Friends",
      registered: "Registered",
      premium_active: "Premium Active",
      pending: "Pending",
      share_invite: "Share Invite",
      copy_link: "Copy Link",
      
      // Social
      add_friends: "Add Friends",
      no_friends_yet: "No activity yet today",
      invite_friends_to_join: "Invite friends to join you",
      meal_logged: "Logged a meal",
      train_with_friends: "Train with friends and boost consistency",
      
      // Precision
      estimated_accuracy: "Estimated Accuracy",
      what_is_precision: "What is 'Estimated Accuracy'?",
      precision_explanation: "It's an estimate of how complete and reliable your data is today. It increases when you log more meals and confirm portions.",
      improve_precision: "Improve Precision",
      precision_factors: "To improve your accuracy:",
      factor_more_meals: "Log 2 more meals",
      factor_confirm_ingredients: "Confirm ingredients",
      factor_clear_photo: "Upload clear photos",
      
      // Errors
      error_uploading_photo: "Couldn't upload photo. Try again",
      error_network: "Network error. Check connection",
      retry: "Retry",
      processing_image: "Processing image...",
      uploading_photo: "Uploading photo...",
      photo_updated: "Photo updated",
      status_updated: "Status updated",
      update_failed: "Update failed",
      settings_updated: "Settings updated",
      
      // Onboarding
      select_language: "Choose your language",
      whats_your_goal: "What's your goal?",
      select_primary_goal: "Select your primary focus",
      choose_pace: "Choose your pace",
      how_challenging: "How challenging should it be?",
      how_use_app: "How do you prefer to use the app?",
      can_change_later: "You can change this later",
      easy_desc: "Gentle pace",
      normal_desc: "Balanced approach",
      challenging_desc: "Push yourself",
      just_me: "Solo",
      just_me_desc: "Private journey",
      with_friends: "With Friends",
      with_friends_desc: "Share progress",
      with_team: "Team",
      with_team_desc: "Join groups",
      safe_mode_active: "Safe Mode: Simplified",

      // Additional translations
      user: "User",
      view_friends: "View Friends",
      friend_activity: "Friend Activity",
      ai_recommendations: "AI Recommendations",
      personalized_tips: "Personalized tips for you",
      notifications: "Notifications",
      gentle_reminders: "Gentle reminders",
      smart_notifications: "Smart behavioral notifications",
      smart_notifications_desc: "Get targeted reminders based on your activity",
      premium_active: "Premium Active",
      all_features_unlocked: "All features unlocked",
      upgrade_to_premium_title: "Upgrade to Premium",
      unlock_all_features: "Unlock all features",
      language_updated: "Language updated",
      
      // Progress
      your_progress: "Your Progress",
      complete_analysis: "Complete analysis of your evolution",
      momentum_score: "Momentum Score",
      never_resets: "Never resets",
      consistency_label: "Consistency",
      adherence_label: "Adherence",
      good_progress_today: "Good progress today. Keep it up.",
      
      // Premium
      upgrade_to_premium: "Upgrade to Premium",
      premium_title: "Go Premium",
      build_consistency: "Build lasting consistency with premium features",
      monthly: "Monthly",
      yearly: "Yearly",
      year: "year",
      month: "month",
      best_value: "Best Value",
      features_premium: "Premium Features",
      fire_system: "Advanced Fire tracking & rewards",
      advanced_ai: "AI-powered nutrition insights",
      progressive_overload: "Progressive overload tracking",
      exclusive_challenges: "Exclusive team challenges",
      analytics: "Advanced analytics dashboard",
      priority_sync: "Priority data sync & backup",
      stories_24h: "24-hour stories (Instagram style)",
      group_collaboration: "Groups & team collaboration",
      progress_tracking_advanced: "Advanced progress tracking",
      nutrition_premium: "Nutrition coaching & guides",
      coaching_personalized: "Personalized AI coaching",
      unlimited_plans: "Unlimited meal & workout plans",
      start_free_trial: "Start Free Trial",
      processing: "Processing...",
      card_required_billing: "Card required, billing starts after trial",
      cancel_anytime: "Cancel anytime, no commitments",
      payment_system_unavailable: "Payment system unavailable",
      please_login_continue: "Please login to continue",
      payment_not_configured: "Payment not configured",
      price_not_available: "Price not available",
      checkout_failed: "Checkout failed",

      // Challenges
      challenges_title: "Challenges",
      challenges_subtitle: "Push yourself. Track progress.",
      my_active_challenges: "My Active Challenges",
      featured_challenges: "Featured Challenges",
      days_label: "days",
      days_remaining: "days remaining",
      joined: "Joined",
      join_challenge: "Join Challenge",
      join_challenge_success: "Joined! 🎉",
      join_challenge_failed: "Failed to join challenge. Try again.",
      premium_feature_title: "Premium Feature",
      challenges_premium_desc: "Challenges are available with your Premium subscription.",
      upgrade_to_premium: "Upgrade to Premium",
      group_challenges: "Group Challenges",
      group_challenges_desc: "Compete with friends in a group",
      challenge_hydration_name: "7-Day Hydration Challenge",
      challenge_hydration_desc: "Drink 8 glasses of water every day for 7 days straight.",
      challenge_streak_name: "30-Day Fitness Streak",
      challenge_streak_desc: "Check in and log movement every single day for 30 days.",
      challenge_meals_name: "21-Day Clean Eating",
      challenge_meals_desc: "Log every meal for 21 days in a row. Awareness is the first step.",
      challenge_workout_name: "10-Workout Sprint",
      challenge_workout_desc: "Complete 10 workouts in under 14 days. Quality over quantity.",
      challenge_consistency_name: "2-Week Consistency",
      challenge_consistency_desc: "Build a solid 14-day streak — log at least one meal each day.",

      // TrialGate / error states
      unable_load_account: "Unable to load your account",
      check_connection_retry: "Check your connection and try again.",
      retry_label: "Retry",
      log_out: "Log out",
      reset_session: "Reset Session",

      // GlobalErrorBoundary / ErrorBoundary
      something_went_wrong: "Something went wrong",
      app_error_desc: "The app encountered an error. Please reload to continue.",
      reload_app: "Reload App",
      error_boundary_title: "Something went wrong",
      error_boundary_desc: "We're working on it. Try reloading the screen.",
      reload: "Reload",
      go_home: "Go Home",

      // Camera extra
      analyzing_ellipsis: "Analyzing…",
      saving_ellipsis: "Saving…",
      initializing_camera_ellipsis: "Initializing camera…",
      center_face_hint: "Center your face and take a photo.",
      profile_photo: "Profile photo",
      barcode_coming_soon: "🔜 Barcode scanning is coming soon. Use Photo or Gallery to log your meal for now.",
      ok: "Got it",

      // Paywall extra
      campus_access_ended: "Campus Access Ended",
      subscribe_continue_progress: "Subscribe to continue your progress.",
      your_trial_ended: "Your Trial Has Ended",
      subscribe_no_interruption: "Subscribe to keep using Balancen without interruption.",
      go_premium: "Go Premium",
      full_access_no_limits: "Full access, no limits.",
      campus_consistency: "Campus Consistency",
      great_work_keep_going: "Great work! Keep the momentum going.",
      subscribe_improve_consistency: "Subscribe to keep improving your consistency.",
      days_tracked: "days tracked",
      meals_logged_stat: "meals logged",
      streak_label: "streak",
      current_streak_label: "current streak",
      best_value_label: "Best Value",
      year_label: "year",
      month_label: "month",
      save_40: "Save 40%",
      processing_ellipsis: "Processing…",
      unlock_premium: "Unlock Premium",
      cancel_anytime_short: "Cancel anytime",
      restore_purchase: "Restore Purchase",
      premium_activated: "Premium activated!",
      purchase_restored: "Purchase restored!",
      no_active_purchase: "No active purchase found.",
      restore_failed: "Restore failed",
      contact_support_restore: "Contact support to restore your purchase.",
      feature_unlimited_ai: "Unlimited AI-powered meal analysis",
      feature_advanced_progress: "Advanced progress tracking",
      feature_groups_social: "Groups & social leaderboard",
      feature_ai_recommendations: "Personalized AI recommendations",
      feature_streak_metrics: "Streak & consistency metrics",
      feature_history_export: "Full history export & backup",
      try_for_free_cta: "Try free — €0.00",
      try_for_free_fine_print: "Free for 5 days, then €8.99/mo or €49.99/yr. Cancel anytime.",

      // Premium hubs and AI features
      progress_saved: "Progress saved!",
      nutrition_hub_premium_title: "Nutrition Hub — Premium",
      nutrition_hub_premium_desc: "Full macro analysis, AI personalized plans and advanced nutrition tracking",
      trainer_premium_title: "AI Trainer — Premium",
      trainer_premium_desc: "Access personalized workout plans, session tracking and progression analysis",
      workout_tracker_premium_title: "Workout Tracker — Premium",
      workout_tracker_premium_desc: "Detailed workout tracking and progression logging",
      plan_generated: "Plan generated by AI",
      plan_error: "Error generating plan",
      plan_activated: "Plan activated",
      story_published: "Story published!",
      ai_scans_limit: "You've used your 5 free AI scans today. Upgrade to Premium for unlimited.",
      ai_scans_left: "AI scans left today",
      daily_limit_reached: "Daily limit reached — Upgrade for unlimited",
      trainer_dashboard_title: "Your AI Trainer",
      always_free_title: "Always free, forever",
      share_meals_public: "Your meals now appear in the Discovery feed 🌍",
      share_meals_private: "Your meals are now private",
      featured_athlete_added: "Marked as Featured Athlete ⭐",
      featured_athlete_removed: "Removed from Featured",
      user_not_found: "User not found",
      profile_copied: "Profile link copied!",
      invite_copied: "Copied!",
      invite_share_text: "Join me on Balancen! Use my code",
      search_foods: "Search foods",
      type_food_name: "Type a food name...",
      search_and_add: "Search & Add",
      selected_foods: "Selected foods",
      total_nutrition: "Total nutrition",
      add_at_least_one_food: "Add at least one food",
      save_failed: "Save failed. Try again.",
      meal_moment_title: "What are you eating now? 📸",
      meal_moment_subtitle: "Share it with your team",
      meal_moment_scan: "Scan",
      invite_description: "You + your friend both get 5 extra Premium days",
      joined_with_link: "joined with your link",
      share_failed: "Share failed",
      }
      },
  nl: {
    translation: {
      // Navigation
      home: "Home",
      social: "Social",
      progress: "Voortgang",
      profile: "Profiel",
      home_tab: "Home",
      social_tab: "Social",
      challenges_tab: "Uitdagingen",
      progress_tab: "Voortgang",
      profile_tab: "Profiel",

      // Camera
      point_camera_at_food: "Richt camera op je voedsel",
      initializing_camera: "Camera starten...",
      camera_not_available: "Camera niet beschikbaar",
      camera_not_supported: "Camera niet beschikbaar. Gebruik de galerij.",
      camera_not_ready: "Camera niet klaar",
      camera_permission_denied: "Cameratoegang geweigerd",
      error_capturing: "Fout bij fotograferen",
      upload_photo: "Foto uploaden",
      cancel: "Annuleren",
      scan_food: "Voedsel scannen",
      barcode: "Barcode",
      gallery: "Galerij",
      save_meal: "Maaltijd opslaan",
      saving: "Opslaan...",
      analyzing: "Analyseren…",
      photo_updated: "Foto bijgewerkt",
      upload_failed: "Upload mislukt",
      profile_photo: "Profielfoto",
      center_face_hint: "Centreer je gezicht en maak een foto.",
      barcode_coming_soon: "🔜 Barcodeherkenning komt binnenkort. Gebruik Foto of Galerij om nu in te loggen.",
      ok: "Begrepen",
      analyzing_ellipsis: "Analyseren…",
      saving_ellipsis: "Opslaan…",
      initializing_camera_ellipsis: "Camera starten…",

      // Challenges
      challenges_title: "Uitdagingen",
      challenges_subtitle: "Daag jezelf uit. Volg voortgang.",
      my_active_challenges: "Mijn actieve uitdagingen",
      featured_challenges: "Aanbevolen uitdagingen",
      days_label: "dagen",
      days_remaining: "dagen resterend",
      joined: "Deelname",
      join_challenge: "Deelnemen aan uitdaging",
      join_challenge_success: "Meedoen! 🎉",
      join_challenge_failed: "Deelname aan uitdaging mislukt. Probeer opnieuw.",
      premium_feature_title: "Premium-functie",
      challenges_premium_desc: "Uitdagingen zijn beschikbaar met je Premium-abonnement.",
      upgrade_to_premium: "Upgrade naar Premium",
      group_challenges: "Groepsuitdagingen",
      group_challenges_desc: "Competeer met vrienden in een groep",
      challenge_hydration_name: "7-daagse hydratatie-uitdaging",
      challenge_hydration_desc: "Drink 8 glazen water elke dag gedurende 7 dagen.",
      challenge_streak_name: "30-daagse fitnesstreak",
      challenge_streak_desc: "Check in en log beweging elke dag gedurende 30 dagen.",
      challenge_meals_name: "21-daagse gezonde voeding",
      challenge_meals_desc: "Log elke maaltijd gedurende 21 dagen.",
      challenge_workout_name: "10-trainingsspurt",
      challenge_workout_desc: "Voltooi 10 trainingen in minder dan 14 dagen.",
      challenge_consistency_name: "2-weekse consistentie",
      challenge_consistency_desc: "Bouw een sterke streak van 14 dagen op.",

      // TrialGate
      unable_load_account: "Kan account niet laden",
      check_connection_retry: "Controleer verbinding en probeer opnieuw.",
      retry_label: "Opnieuw proberen",
      log_out: "Uitloggen",
      reset_session: "Sessie herstellen",

      // Error Boundaries
      something_went_wrong: "Er is iets misgegaan",
      app_error_desc: "De app ondervond een fout. Herlaad om door te gaan.",
      reload_app: "App herladen",
      error_boundary_title: "Er is iets misgegaan",
      error_boundary_desc: "We werken eraan. Probeer het scherm opnieuw te laden.",
      reload: "Herladen",
      go_home: "Terug naar home",

      // Paywall
      campus_access_ended: "Campus-toegang beëindigd",
      subscribe_continue_progress: "Abonneer je om je voortgang voort te zetten.",
      your_trial_ended: "Je proefperiode is afgelopen",
      subscribe_no_interruption: "Abonneer je om Balancen zonder onderbreking te gebruiken.",
      go_premium: "Ga Premium",
      full_access_no_limits: "Volledige toegang, geen limieten.",
      campus_consistency: "Campus-consistentie",
      great_work_keep_going: "Goed gedaan! Ga door.",
      subscribe_improve_consistency: "Abonneer je om je consistentie te verbeteren.",
      days_tracked: "dagen bijgehouden",
      meals_logged_stat: "maaltijden gelogd",
      streak_label: "streak",
      current_streak_label: "huidige streak",
      best_value_label: "Beste waarde",
      year_label: "jaar",
      month_label: "maand",
      save_40: "Bespaar 40%",
      processing_ellipsis: "Verwerken…",
      unlock_premium: "Premium ontgrendelen",
      cancel_anytime_short: "Annuleer wanneer je wilt",
      restore_purchase: "Aankoop herstellen",
      premium_activated: "Premium geactiveerd!",
      purchase_restored: "Aankoop hersteld!",
      no_active_purchase: "Geen actieve aankoop gevonden.",
      restore_failed: "Herstel mislukt",
      contact_support_restore: "Neem contact op met ondersteuning om je aankoop te herstellen.",
      feature_unlimited_ai: "Onbeperkte AI-voedselanalyse",
      feature_advanced_progress: "Geavanceerde voortgangstracking",
      feature_groups_social: "Groepen en sociale leaderboard",
      feature_ai_recommendations: "Persoonlijke AI-aanbevelingen",
      feature_streak_metrics: "Streak- en consistentiemetrieken",
      feature_history_export: "Volledige geschiedenisexport",
      try_for_free_cta: "Gratis proberen — €0,00",
      try_for_free_fine_print: "Gratis 5 dagen, daarna €8,99/maand of €49,99/jaar. Annuleer wanneer je wilt.",

      // General
      logout: "Uitloggen",
      settings: "Instellingen",
      settings_desc: "Taal, privacy en meer",
      language: "Taal",
      select_language: "Taal selecteren",
      continue: "Doorgaan",
      loading: "Laden...",
      please_login_continue: "Meld je aan om door te gaan",
      payment_not_configured: "Betaling niet geconfigureerd",
      language_updated: "Taal bijgewerkt",
      save: "Opslaan",
      cancel: "Annuleren",
      name: "Naam",
      enter_your_name: "Je naam",
      personal_info: "Persoonlijke informatie",
      name_updated: "Naam bijgewerkt",
      update_failed: "Update mislukt",
      status_updated: "Status bijgewerkt",
      status_placeholder: "Waar denk je aan?",
      status: "Status",
      your_goals: "Je doelen",
      main_goal: "Hoofddoel",
      intensity: "Intensiteit",
      daily_goal: "Dagelijks doel",
      not_defined: "Niet ingesteld",
      edit_goals: "Doelen bewerken",
      consistency: "Consistenter worden",
      weight_loss: "Gewicht verliezen",
      healthy_habits: "Gezonde gewoonten",
      stay_active: "Actief blijven",
      easy: "Makkelijk",
      normal: "Normaal",
      challenging: "Uitdagend",
      current_streak: "Huidige streak",
      best_streak: "Beste streak",
      total_checkins: "Totale check-ins",
      ai_goals_assistant: "AI-doelhulp",
      ai_goals_desc: "Krijg gepersonaliseerde aanbevelingen",
      profile: "Profiel",
      create_your_profile: "Maak je profiel",
      sign_in_to_access_profile: "Meld je aan voor je profiel",
      sign_in: "Aanmelden",
      premium_active: "Premium actief",
      collaborator: "Medewerker",
      photo_updated: "Foto bijgewerkt",
      upload_failed: "Upload mislukt",
      meal_moment_title: "Wat eet je nu? 📸",
      meal_moment_subtitle: "Deel het met je team",
      meal_moment_scan: "Scan",
      ai_scans_limit: "Je hebt vandaag je 5 gratis AI-scans gebruikt. Upgrade naar Premium voor onbeperkt.",
      invite_description: "Jij en je vriend krijgen beide 5 extra Premium-dagen",
      joined_with_link: "mededaan met je link",
      share_failed: "Delen mislukt",
      search_foods: "Zoek voedsel",
      type_food_name: "Typ een voedsel...",
      search_and_add: "Zoeken & toevoegen",
      selected_foods: "Geselecteerde voedingsmiddelen",
      total_nutrition: "Totale voeding",
      add_at_least_one_food: "Voeg ten minste één voedsel toe",
      save_failed: "Opslaan mislukt. Probeer opnieuw.",
      keep_it_up: "Ga door",
      canvas: "canvas",
      }
      },
  es: {
    translation: {
      // Navigation
      home: "Inicio",
      social: "Social",
      progress: "Progreso",
      profile: "Perfil",
      home_tab: "Inicio",
      social_tab: "Social",
      challenges_tab: "Retos",
      progress_tab: "Progreso",
      profile_tab: "Perfil",
      
      // Camera flow
      point_camera_at_food: "Apunta la cámara a tu comida",
      initializing_camera: "Iniciando cámara...",
      couldnt_recognize_food: "No pudimos reconocer esta comida",
      try_again_or_manual: "Intenta otra foto o añádela manualmente",
      retry_photo: "Tomar Otra Foto",
      add_manually: "Añadir Manualmente",
      review_and_confirm: "Revisa y confirma los detalles",
      total_calories: "Calorías Totales",
      tap_to_edit: "Toca para editar si es necesario",
      photo_looks_good: "¿Se ve bien?",
      retake: "Repetir",
      use_photo: "Usar Foto",
      // Analysis loader steps
      uploading_photo: "Subiendo foto...",
      reading_photo: "Leyendo tu foto...",
      identifying_foods: "Identificando alimentos...",
      calculating_nutrition: "Calculando nutrición...",
      finalizing: "Casi listo...",
      analyzing_meal: "Analizando comida",
      please_wait: "Por favor espera...",
      // Review screen
      review_meal: "Revisar Comida",
      confidence: "confianza",
      confirm_save: "Confirmar y Guardar",
      
      // Home
      current_streak: "Racha Actual",
      days_in_a_row: "¡Sigue así!",
      total_fire: "Fuego Total",
      today_progress: "Mantén tu ritmo",
      carbs: "Carbohidratos",
      fats: "Grasas",
      calories: "Calorías",
      protein: "Proteína",
      meals_logged_today: "Comidas registradas hoy",
      log_your_meal: "Registrar Comida",
      friends_active: "Amigos activos hoy",
      friends: "amigos",
      recent_activity: "Actividad Reciente",
      meal: "Comida",
      meal_logged: "¡Comida registrada!",
      meal_saved: "¡Comida Guardada!",
      great_job: "¡Excelente trabajo!",
      logged_meals: "Comidas Registradas",
      social_feed: "Feed Social",
      active_friends: "amigos activos",
      ai_coach: "IA Coach",
      ai_coach_goal_reached: "¡Increíble trabajo! Alcanzaste tu meta de calorías. Continúa con esta consistencia para maximizar resultados.",
      ai_coach_halfway: "Buen progreso. Estás a mitad de camino. Registra tu siguiente comida para llegar a la meta.",
      ai_coach_start: "Comienza registrando tus comidas. Cada foto te acerca a entender mejor tu nutrición.",
      todays_missions: "Misiones de Hoy",
      last_meal: "Última Comida",
      stay_within_goal: "Cumplir meta de calorías",
      log_three_meals: "Registrar 3 comidas",
      edit_goals: "Editar Objetivos",
      save_changes: "Guardar Cambios",
      goal_updated: "Objetivos actualizados",
      primary: "Principal",
      secondary: "Secundario",
      private_mode: "Privado",
      friends_mode: "Solo Amigos",
      groups_mode: "Grupos",
      
      // Paywall
      choose_plan: "¿Hasta dónde quieres llegar?",
      free_lets_start: "Gratis te deja empezar. Premium te lleva más lejos.",
      free_plan: "Plan Gratis",
      premium_plan: "Plan Premium",
      includes: "Incluye",
      not_included: "No incluye",
      full_experience: "Experiencia Completa",
      ai_coaching_helps: "Coaching IA te ayuda a:",
      review_activity: "Revisar actividad y metas",
      give_recommendations: "Dar recomendaciones personalizadas",
      adjust_habits: "Ajustar hábitos con el tiempo",
      support_consistency: "Apoyar tu consistencia",
      premium_for_lasting: "Premium está diseñado para construir consistencia duradera.",
      free_for_trial: "Gratis está diseñado para probar. Premium para resultados reales.",
      premium_pricing: "Precio Premium",
      continue_free_limited: "Continuar con versión gratuita (limitada)",
      
      // Free features
      daily_checkins: "Check-ins diarios",
      basic_fire_tracking: "Seguimiento básico de racha",
      manual_tracking: "Registro manual",
      one_group_max: "Máx. 1 grupo",
      basic_stats: "Estadísticas básicas",
      
      // Free limitations
      streak_capped: "Racha limitada a 3 días",
      limited_fire_system: "Sistema de fuego limitado",
      no_auto_goals: "Sin progresión automática",
      no_ai_coaching: "Sin coaching IA",
      no_advanced_analytics: "Sin analíticas avanzadas",
      no_progressive_challenges: "Sin desafíos progresivos",
      limited_social: "Funciones sociales limitadas",
      
      // Premium features
      unlimited_streaks: "Rachas y fuego ilimitado",
      three_fire_metrics: "Tres métricas de fuego",
      auto_goal_progression: "Progresión automática de metas",
      ai_coaching_personalized: "Coaching IA personalizado",
      ai_meal_planner_feature: "Planes de comidas IA personalizados",
      ai_workout_generator_feature: "Planes de entrenamiento IA adaptativos",
      social_feed_feature: "Feed social y publicaciones",
      enhanced_friends_feature: "Sistema de amigos mejorado",
      advanced_analytics: "Analíticas e insights avanzados",
      unlimited_groups: "Grupos ilimitados",
      social_leaderboards: "Tablas de clasificación social",
      priority_sync: "Sincronización prioritaria",
      full_history_export: "Exportación de historial completo",
      progressive_challenges: "Desafíos progresivos",
      unlock_social_feed: "Desbloquea el feed social para compartir y conectar con amigos",
      unlock_friends_features: "Desbloquea funciones de amigos para conectar con otros",
      
      // Camera labels
      scan_food: "Escanear Comida",
      barcode: "Código de Barras",
      gallery: "Galería",

      // Profile
      language: "Idioma",
      photo_updated: "Foto actualizada",

      // Misc
      role: "Rol",
      collaborator: "Colaborador",
      estimated: "estimado",
      cal: "cal",
      invite_share_title: "Únete a mí en la app",
      invite_share_text: "¡Hola! Te invito a unirte a esta app de bienestar. ¡Alcancemos nuestros objetivos juntos! 🔥",
      invite_shared: "Invitación compartida",
      link_copied: "Link copiado",
      invite_error: "Error al generar invitación",
      copy_failed: "Error al copiar",
      referral_reward_text: "Por cada 3 amigos que se suscriban a Premium, ganás 1 mes gratis",
      progress: "Progreso",
      friends_converted: "amigos convertidos",
      
      // Friends page
      friends_title: "Amigos",
      connected: "conectados",
      add_friend: "Agregar Amigo",
      friend_email: "Email del amigo",
      send_request: "Enviar Solicitud",
      pending_requests: "Solicitudes Pendientes",
      wants_to_connect: "quiere conectarse",
      my_friends: "Mis Amigos",
      add_friends_to_share: "Agrega amigos para compartir tu progreso",
      friend_request_sent: "Solicitud enviada",
      request_updated: "Solicitud actualizada",
      day_streak: "días de racha",
      meals: "comidas",
      last_meal_label: "Última comida",
      
      // Social Feed
      share_your_progress: "Comparte tu progreso con amigos",
      whats_on_your_mind: "¿Qué estás pensando?",
      no_posts_yet: "Aún no hay publicaciones",
      be_first_to_share: "¡Sé el primero en compartir tu progreso!",
      create_post: "Crear Publicación",
      add_content_or_image: "Agrega contenido o una imagen",
      post_created: "Publicación creada",
      post_failed: "Error al crear publicación",
      just_now: "Justo ahora",
      add_comment: "Agrega un comentario...",
      post: "Publicar",
      comment_added: "Comentario agregado",
      error_adding_comment: "Error al agregar comentario",
      add_image: "Agregar Imagen",
      uploading: "Subiendo...",
      achievement: "Logro",
      upload_failed: "Error al subir",
      image_uploaded: "Imagen subida",
      share_and_connect: "Comparte progreso con amigos",
      
      // AI Goals Assistant
      ai_goals_assistant: "Asistente IA de Objetivos",
      ai_goals_desc: "Obtén recomendaciones personalizadas",
      premium_feature: "Función Premium",
      ai_goals_premium_desc: "Desbloquea recomendaciones personalizadas de objetivos con IA con Premium",
      current_goals: "Tus Objetivos Actuales",
      daily_calories: "Calorías Diarias",
      not_set: "No definido",
      get_ai_recommendations: "Obtener Recomendaciones IA",
      analyzing: "Analizando tus datos...",
      personalized_insight: "Insight Personalizado",
      recommended_goals: "Objetivos Recomendados",
      based_on_your_activity: "Basado en tu actividad y objetivos",
      activity_goal: "Objetivo de Actividad",
      days_per_week: "días/semana",
      reanalyze: "Reanalizar",
      apply_goals: "Aplicar Objetivos",
      goals_updated: "Objetivos actualizados",
      analysis_failed: "Análisis falló",
      
      // AI Coaching Tools
      ai_coaching_tools: "Herramientas IA de Coaching",
      ai_meal_planner: "Planificador IA de Comidas",
      personalized_meal_plans: "Planes de comidas personalizados",
      dietary_restrictions: "Restricciones Dietéticas",
      dietary_placeholder: "ej., vegano, sin gluten",
      food_preferences: "Preferencias Alimentarias",
      preferences_placeholder: "ej., ama el pollo, no mariscos",
      plan_duration: "Duración del Plan",
      days: "días",
      generate_meal_plan: "Generar Plan de Comidas",
      generating: "Generando...",
      meal_plan_generated: "Plan de comidas generado",
      generation_failed: "Error al generar",
      your_meal_plan: "Tu Plan de Comidas",
      day: "Día",
      shopping_list: "Lista de Compras",
      tips: "Consejos",
      close: "Cerrar",
      
      ai_workout_generator: "Generador IA de Entrenamientos",
      adaptive_workout_plans: "Planes de entrenamiento adaptativos",
      available_equipment: "Equipamiento Disponible",
      equipment_placeholder: "ej., mancuernas, bandas de resistencia",
      fitness_level: "Nivel de Fitness",
      beginner: "Principiante",
      intermediate: "Intermedio",
      advanced: "Avanzado",
      workout_goals: "Objetivos de Entrenamiento",
      goals_placeholder: "ej., ganar músculo, perder grasa",
      session_duration: "Duración de Sesión",
      minutes: "min",
      generate_workout: "Generar Entrenamiento",
      workout_generated: "Entrenamiento generado",
      your_workout: "Tu Entrenamiento",
      warmup: "Calentamiento",
      main_workout: "Entrenamiento Principal",
      cooldown: "Enfriamiento",
      sets: "series",
      rest: "Descanso",
      weeks: "Semanas",
      
      // Groups page
      groups: "Grupos",
      groups_joined: "grupos unidos",
      create_group: "Crear Grupo",
      join_group: "Unirse a Grupo",
      group_name_placeholder: "Nombre del grupo",
      create: "Crear",
      join: "Unirse",
      invite_code_placeholder: "Código de invitación",
      group_created: "Grupo creado",
      joined_group: "Te uniste al grupo",
      code_copied: "Código copiado",
      no_groups_yet: "Aún sin grupos",
      create_or_join_group: "Crea o únete a un grupo para empezar",
      members: "miembros",
      friend_request_exists: "Solicitud de amistad ya existe",
      group_not_found: "Grupo no encontrado",
      
      // Premium page
      already_have_premium: "Ya tienes acceso Premium",
      
      // UserManagement
      user_management: "Gestión de Usuarios",
      view_manage_users: "Ver y gestionar todos los usuarios",
      total_users: "Total Usuarios",
      premium_users: "Premium",
      collaborators_count: "Colaboradores",
      search_users: "Buscar usuarios...",
      premium_access: "Acceso Premium",
      go_back: "Volver",
      access_denied_owner: "Acceso denegado - solo propietario.",
      forbidden: "403 Prohibido",
      premium_status_updated: "Estado Premium actualizado",
      
      // InviteCollaborators
      invite_collaborators_title: "Invitar Colaboradores",
      grant_free_premium: "Dar acceso Premium gratuito",
      new_invitation: "Nueva Invitación",
      email_placeholder_collab: "email@ejemplo.com",
      sending: "Enviando...",
      send_invitation: "Enviar Invitación",
      sent_invitations: "Invitaciones Enviadas",
      no_invitations_yet: "Aún no has enviado invitaciones",
      pending_status: "Pendiente",
      accepted_status: "Aceptada",
      rejected_status: "Rechazada",
      invalid_email: "Email inválido",
      cannot_invite_yourself: "No puedes invitarte a ti mismo",
      already_invited_email: "Ya invitaste este email",
      invitation_sent: "Invitación enviada",
      invitation_error: "Error al enviar invitación",
      only_owner_can_invite: "Solo el propietario puede invitar colaboradores",
      premium_invitation_subject: "Invitación a Balancen Premium",
      premium_invitation_body: "Has sido invitado a unirte a Balancen con acceso Premium gratuito. Por favor, regístrate en la app usando este email: {{email}}",
      
      // Home redesign
      good_morning: "Buenos días",
      good_afternoon: "Buenas tardes",
      good_evening: "Buenas noches",
      daily_intake: "Ingesta Diaria",
      recent_meals: "Comidas Recientes",
      no_meals_logged: "Aún no hay comidas registradas",
      start_logging: "Empieza a registrar para rastrear tu nutrición",
      streak_momentum: "Racha & Momentum",
      best_streak: "Mejor",
      ai_daily_insight: "Insight IA",
      unlock_ai_insights: "Desbloquea insights IA con Premium",
      todays_missions: "Misiones de Hoy",
      view_all: "Ver todo",
      kcal_short: "kcal",
      gram_short: "g",
      log_meal_button: "Comida",
      workout_button: "Entreno",
      manual_log_description: "Busca y añade comidas manualmente sin tomar foto",
      premium_features: "Funciones Premium",
      premium_features_description: "Coach IA, Social, Grupos, Analíticas Avanzadas y Más",
      log_meal_manually: "Registrar Comida Manualmente",
      
      // Social features
      your_groups: "Tus Grupos",
      create_group_btn: "Crear Grupo",
      join_group_btn: "Unirse a Grupo",
      leaderboard: "Tabla de Posiciones",
      feed: "Feed",
      this_week: "Esta Semana",
      fire_points: "Puntos de Fuego",
      no_posts_yet: "Aún no hay publicaciones",
      share_something: "Comparte tu progreso con el grupo",
      post_meal: "Compartir Comida",
      post_status: "Publicar Estado",
      locked_feature: "Función Premium",
      unlock_social: "Desbloquea grupos, tabla y feed con Premium",
      upgrade_now: "Mejorar Ahora",
      react: "Reaccionar",
      comment: "Comentar",
      members_count: "{{count}} miembros",
      
      // Meal logging
      log_meal: "Registrar Comida",
      take_photo: "Tomar Foto",
      choose_gallery: "Elegir de Galería",
      manual_entry: "Entrada Manual",
      meal_details: "Detalles de Comida",
      
      // Empty states
      no_groups_joined: "Aún no te uniste a ningún grupo",
      create_join_group: "Crea o únete a un grupo para empezar a competir",
      
      // Social tab
      connect_compete_share: "Conecta, compite y comparte progreso",
      meal_sharing_disabled: "Compartir comidas está deshabilitado en tu privacidad",
      no_members_yet: "Aún no hay miembros",
      unknown_user: "Usuario Desconocido",
      share_invite: "Compartir",
      no_data_yet: "Aún no hay datos. ¡Empieza a registrar comidas!",
      loading: "Cargando...",
      track_your_journey: "Rastrea tu viaje y métricas",
      your_progress: "Tu Progreso",
      complete_analysis: "Análisis completo de tu nutrición",
      momentum_score: "Puntaje de Momentum",
      never_resets: "Nunca se reinicia",
      consistency_label: "Consistencia",
      adherence_label: "Adherencia",
      good_progress_today: "¡Buen progreso hoy! Sigue así.",
      invite_code: "Código de Invitación",
      set_status: "Establecer Estado",
      active_challenges: "Desafíos Activos",
      goal: "Meta",
      ends: "Termina",
      status_placeholder: "¿Qué estás pensando?",
      characters: "caracteres",
      clear_status: "Limpiar Estado",
      status_cleared: "Estado limpiado",
      status_too_long: "Estado demasiado largo (máx 32 caracteres)",
      status_inappropriate: "El estado contiene lenguaje inapropiado",
      
      // Onboarding
      select_language: "Seleccionar Idioma",
      select_your_language: "Selecciona tu idioma",
      continue: "Continuar",
      whats_main_goal: "¿Cuál es tu meta principal?",
      choose_intensity: "Elige tu intensidad",
      set_calorie_limit: "Establece tu límite de calorías diarias",
      finish_setup: "Finalizar Configuración",
      skip_for_now: "Omitir por ahora",
      
      // Delete account
      delete_account_title: "¿Estás seguro?",
      delete_account_message: "¿Estás seguro de que quieres eliminar tu cuenta? Esta acción no se puede deshacer.",
      delete_warning: "Esta acción es irreversible. Todos tus datos serán eliminados.",
      type_delete: "Escribe DELETE",
      type_delete_confirm: "Escribe DELETE para confirmar",
      account_deleted: "Cuenta eliminada",
      delete_account_error: "Error al eliminar cuenta",
      deleting: "Eliminando...",
      delete: "Eliminar",
      
      // Camera
      camera_access_denied: "Acceso a cámara denegado",
      camera_permission_denied: "Permiso de cámara denegado",
      camera_not_available: "Cámara no disponible",
      camera_not_supported: "Cámara no disponible. Usa la galería.",
      camera_not_ready: "Cámara no lista",
      error_capturing: "Error al capturar foto",
      upload_photo: "Subir Foto",
      analyzing_meal: "Analizando comida...",
      please_wait: "Por favor espera",
      analysis_failed: "Análisis falló",
      detected_items: "Items Detectados",
      meal_analysis: "Análisis de Comida",
      save_meal: "Guardar Comida",
      saving: "Guardando...",
      error_saving: "Error al guardar comida",
      upload_photo_instead: "Subir una foto",
      cancel: "Cancelar",
      choose_from_gallery: "Elegir de la Galería",
      take_photo: "Sacar una Foto",
      scan_food: "Escanear Comida",
      barcode: "Código de Barras",
      gallery: "Galería",
      
      // Profile
      my_profile: "Mi Perfil",
      edit: "Editar",
      save: "Guardar",
      best_streak: "Mejor Racha",
      total_checkins: "Check-ins Totales",
      add_status: "Agregar estado",
      tap_to_edit: "Toca para editar",
      status_expires_24h: "Expira en 24h",
      settings: "Configuración",
      settings_desc: "Idioma, privacidad y más",
      your_goals: "Tus Objetivos",
      main_goal: "Objetivo Principal",
      intensity: "Intensidad",
      mode: "Modo",
      with_friends: "Con Amigos",
      alone: "Solo",
      optional_data: "Datos Opcionales",
      weight_kg: "Peso (kg)",
      height_cm: "Altura (cm)",
      optional: "Opcional",
      not_defined: "No definido",
      logout: "Cerrar Sesión",
      status: "Estado",
      status_placeholder: "¿Qué estás pensando?",
      your_goal: "Tu Meta",
      primary_goal: "Objetivo principal",
      daily_goal: "Meta diaria",
      
      // Goals
      consistency: "Ser más consistente",
      weight_loss: "Bajar de peso",
      healthy_habits: "Hábitos saludables",
      stay_active: "Mantenerse activo",
      
      // Intensity
      easy: "Fácil",
      easy_desc: "Ritmo suave",
      normal: "Normal",
      normal_desc: "Enfoque balanceado",
      challenging: "Desafiante",
      challenging_desc: "Desafíate a vos mismo",
      
      // Settings
      language: "Idioma",
      select_language: "Seleccionar Idioma",
      privacy: "Privacidad",
      data_encrypted: "Tus datos están protegidos",
      admin_tools: "Herramientas de Admin",
      invite_collaborators: "Invitar Colaboradores",
      grant_free_premium: "Dar Acceso Premium Gratis",
      user_management: "Gestión de Usuarios",
      view_manage_users: "Ver y administrar usuarios",
      daily_calories_limit: "Límite diario de calorías",
      optional_leave_empty: "Opcional - dejalo vacío si no estás seguro",
      goals_targets: "Objetivos y Metas",
      set_daily_goals: "Configurar objetivos diarios",
      session_expired: "Sesión expirada",
      please_login_again: "Por favor ingresá de nuevo",
      connection_error: "Error de conexión",
      check_internet: "Revisá tu conexión a internet",
      
      // Referral
      invite_friends: "Invitar Amigos",
      your_referrals: "Tus referidos",
      free_month_progress: "para tu próximo mes gratis",
      
      // Premium differentiation
      free_limited_history: "Solo datos de hoy",
      premium_full_history: "Historial completo y análisis",
      week_view: "Vista Semanal",
      month_view: "Vista Mensual",
      year_view: "Vista Anual",
      ai_food_recognition: "Reconocimiento de comida con IA",
      adaptive_targets: "Objetivos adaptativos de calorías y macros",
      social_competition: "Competencia social y equipos",
      advanced_dashboards: "Dashboards avanzados",
      invited_friends: "Amigos Invitados",
      registered: "Registrado",
      premium_active: "Premium Activo",
      pending: "Pendiente",
      share_invite: "Compartir Invitación",
      copy_link: "Copiar Link",
      
      // Social
      add_friends: "Agregar Amigos",
      no_friends_yet: "Aún sin actividad hoy",
      invite_friends_to_join: "Invitá amigos a unirse",
      meal_logged: "Registró una comida",
      train_with_friends: "Entrena con amigos y aumenta tu constancia",
      
      // Precision
      estimated_accuracy: "Precisión Estimada",
      what_is_precision: "¿Qué significa 'Precisión Estimada'?",
      precision_explanation: "Es una estimación de cuán completos y confiables son tus datos de hoy. Sube cuando registrás más comidas y confirmás porciones.",
      improve_precision: "Mejorar Precisión",
      precision_factors: "Para mejorar tu precisión:",
      factor_more_meals: "Registrá 2 comidas más",
      factor_confirm_ingredients: "Confirmá ingredientes",
      factor_clear_photo: "Subí fotos nítidas",
      
      // Errors
      error_uploading_photo: "No pudimos subir la foto. Intentá otra vez",
      error_network: "Error de red. Revisá tu conexión",
      retry: "Reintentar",
      processing_image: "Procesando imagen...",
      uploading_photo: "Subiendo foto...",
      photo_updated: "Foto actualizada",
      status_updated: "Estado actualizado",
      update_failed: "Error al actualizar",
      settings_updated: "Configuración actualizada",
      
      // Onboarding
      select_language: "Elige tu idioma",
      whats_your_goal: "¿Cuál es tu objetivo?",
      select_primary_goal: "Selecciona tu enfoque principal",
      choose_pace: "Elige tu ritmo",
      how_challenging: "¿Qué tan desafiante debe ser?",
      how_use_app: "¿Cómo prefieres usar la app?",
      can_change_later: "Podés cambiar esto después",
      easy_desc: "Ritmo suave",
      normal_desc: "Enfoque balanceado",
      challenging_desc: "Desafíate a vos mismo",
      just_me: "Solo",
      just_me_desc: "Viaje privado",
      with_friends: "Con Amigos",
      with_friends_desc: "Compartir progreso",
      with_team: "Equipo",
      with_team_desc: "Únete a grupos",
      safe_mode_active: "Modo Seguro: Simplificado",

      // Additional translations
      user: "Usuario",
      view_friends: "Ver Amigos",
      friend_activity: "Actividad de Amigos",
      ai_recommendations: "Recomendaciones IA",
      personalized_tips: "Tips personalizados para vos",
      notifications: "Notificaciones",
      gentle_reminders: "Recordatorios suaves",
      smart_notifications: "Notificaciones inteligentes",
      smart_notifications_desc: "Obtén recordatorios dirigidos según tu actividad",
      premium_active: "Premium Activo",
      all_features_unlocked: "Todas las funciones desbloqueadas",
      upgrade_to_premium_title: "Actualizar a Premium",
      unlock_all_features: "Desbloquear todas las funciones",
      language_updated: "Idioma actualizado",
      name_updated: "Nombre actualizado",
      personal_info: "Información personal",
      name: "Nombre",
      enter_your_name: "Tu nombre",
      
      // Progress
      your_progress: "Tu Progreso",
      complete_analysis: "Análisis completo de tu evolución",
      momentum_score: "Momentum Score",
      never_resets: "Nunca vuelve a cero",
      consistency_label: "Consistencia",
      adherence_label: "Adherencia",
      good_progress_today: "Buen progreso hoy. Mantén el ritmo.",
      
      // Premium
      upgrade_to_premium: "Actualizar a Premium",
      premium_title: "Pasa a Premium",
      build_consistency: "Construye consistencia duradera con funciones premium",
      monthly: "Mensual",
      yearly: "Anual",
      year: "año",
      month: "mes",
      best_value: "Mejor Valor",
      features_premium: "Funciones Premium",
      fire_system: "Sistema avanzado de Fire y recompensas",
      advanced_ai: "Insights nutricionales con IA",
      progressive_overload: "Seguimiento de sobrecarga progresiva",
      exclusive_challenges: "Desafíos exclusivos en equipo",
      analytics: "Dashboard de analíticas avanzado",
      priority_sync: "Sincronización y backup prioritarios",
      stories_24h: "Historias de 24h (estilo Instagram)",
      group_collaboration: "Grupos y colaboración en equipo",
      progress_tracking_advanced: "Rastreo de progreso avanzado",
      nutrition_premium: "Coaching y guías nutricionales",
      coaching_personalized: "Coaching IA personalizado",
      unlimited_plans: "Planes ilimitados de comidas y entrenamientos",
      start_free_trial: "Comenzar Prueba Gratis",
      processing: "Procesando...",
      card_required_billing: "Tarjeta requerida, facturación inicia después del trial",
      cancel_anytime: "Cancelar en cualquier momento, sin compromisos",
      payment_system_unavailable: "Sistema de pago no disponible",
      please_login_continue: "Por favor inicia sesión para continuar",
      payment_not_configured: "Pago no configurado",
      price_not_available: "Precio no disponible",
      checkout_failed: "Checkout falló",

      // Challenges
      challenges_title: "Desafíos",
      challenges_subtitle: "Superáte. Seguí tu progreso.",
      my_active_challenges: "Mis Desafíos Activos",
      featured_challenges: "Desafíos Destacados",
      days_label: "días",
      days_remaining: "días restantes",
      joined: "Unido",
      join_challenge: "Unirse al Desafío",
      join_challenge_success: "¡Te uniste! 🎉",
      join_challenge_failed: "No se pudo unir al desafío. Intentá de nuevo.",
      premium_feature_title: "Función Premium",
      challenges_premium_desc: "Los desafíos están disponibles con tu suscripción Premium.",
      group_challenges: "Desafíos de Grupo",
      group_challenges_desc: "Competí con amigos en un grupo",
      challenge_hydration_name: "Desafío Hidratación 7 Días",
      challenge_hydration_desc: "Tomá 8 vasos de agua todos los días durante 7 días seguidos.",
      challenge_streak_name: "Racha Fitness 30 Días",
      challenge_streak_desc: "Hacé check-in y registrá movimiento todos los días por 30 días.",
      challenge_meals_name: "Alimentación Limpia 21 Días",
      challenge_meals_desc: "Registrá cada comida por 21 días seguidos. La conciencia es el primer paso.",
      challenge_workout_name: "Sprint de 10 Entrenamientos",
      challenge_workout_desc: "Completá 10 entrenamientos en menos de 14 días. Calidad sobre cantidad.",
      challenge_consistency_name: "Consistencia 2 Semanas",
      challenge_consistency_desc: "Construí una racha sólida de 14 días — registrá al menos una comida por día.",

      // TrialGate
      unable_load_account: "No se pudo cargar tu cuenta",
      check_connection_retry: "Revisá tu conexión e intentá de nuevo.",
      retry_label: "Reintentar",
      log_out: "Cerrar sesión",
      reset_session: "Restablecer Sesión",

      // Error Boundaries
      something_went_wrong: "Algo salió mal",
      app_error_desc: "La app encontró un error. Recargá para continuar.",
      reload_app: "Recargar App",
      error_boundary_title: "Algo salió mal",
      error_boundary_desc: "Estamos trabajando para solucionarlo. Intentá recargar la pantalla.",
      reload: "Recargar",
      go_home: "Volver al inicio",

      // Camera extra
      analyzing_ellipsis: "Analizando…",
      saving_ellipsis: "Guardando…",
      initializing_camera_ellipsis: "Iniciando cámara…",
      center_face_hint: "Centrá tu cara y sacate una foto.",
      profile_photo: "Foto de perfil",
      barcode_coming_soon: "🔜 El escaneo de código de barras llegará pronto. Usá Foto o Galería para registrar tu comida.",
      ok: "Entendido",

      // Paywall extra
      campus_access_ended: "Tu Campus Access ha terminado",
      subscribe_continue_progress: "Suscribite para seguir con tu progreso.",
      your_trial_ended: "Tu Trial ha terminado",
      subscribe_no_interruption: "Suscribite para seguir usando Balancen sin interrupciones.",
      go_premium: "Hazte Premium",
      full_access_no_limits: "Acceso completo, sin límites.",
      campus_consistency: "Consistencia del Campus",
      great_work_keep_going: "¡Excelente trabajo! Seguí con el impulso.",
      subscribe_improve_consistency: "Suscribite para seguir mejorando tu consistencia.",
      days_tracked: "días rastreados",
      meals_logged_stat: "comidas registradas",
      streak_label: "racha",
      current_streak_label: "racha actual",
      best_value_label: "Mejor Valor",
      year_label: "año",
      month_label: "mes",
      save_40: "Ahorrá un 40%",
      processing_ellipsis: "Procesando…",
      unlock_premium: "Desbloquear Premium",
      cancel_anytime_short: "Cancelá cuando quieras",
      restore_purchase: "Restaurar Compra",
      premium_activated: "¡Premium activado!",
      purchase_restored: "¡Compra restaurada!",
      no_active_purchase: "No se encontró ninguna compra activa.",
      restore_failed: "No se pudo restaurar",
      contact_support_restore: "Contactá soporte para restaurar tu compra.",
      feature_unlimited_ai: "Análisis de comidas con IA ilimitado",
      feature_advanced_progress: "Seguimiento avanzado de progreso",
      feature_groups_social: "Grupos y leaderboard social",
      feature_ai_recommendations: "Recomendaciones personalizadas de IA",
      feature_streak_metrics: "Racha y métricas de consistencia",
      feature_history_export: "Exportación de historial completo",
      try_for_free_cta: "Probar gratis — €0.00",
      try_for_free_fine_print: "Gratis 5 días, luego €8.99/mes o €49.99/año. Cancela cuando quieras.",

      // Premium hubs and AI features
      progress_saved: "¡Progreso guardado!",
      nutrition_hub_premium_title: "Centro Nutricional — Premium",
      nutrition_hub_premium_desc: "Análisis completo de macros, planes personalizados por IA y seguimiento nutricional avanzado",
      trainer_premium_title: "Entrenador IA — Premium",
      trainer_premium_desc: "Acceso a planes de entrenamiento personalizados, seguimiento de sesiones y análisis de progresión",
      workout_tracker_premium_title: "Registro de Entrenamientos — Premium",
      workout_tracker_premium_desc: "Seguimiento detallado de entrenamientos y progresión",
      plan_generated: "Plan generado por IA",
      plan_error: "Error al generar plan",
      plan_activated: "Plan activado",
      story_published: "¡Historia publicada!",
      ai_scans_limit: "Usaste tus 5 análisis IA gratuitos de hoy. Actualiza a Premium para ilimitados.",
      ai_scans_left: "análisis IA restantes hoy",
      daily_limit_reached: "Límite diario alcanzado — Actualiza para ilimitado",
      trainer_dashboard_title: "Tu Entrenador IA",
      always_free_title: "Siempre gratis, para siempre",
      share_meals_public: "Tus comidas ahora aparecen en el feed de descubrimiento 🌍",
      share_meals_private: "Tus comidas ahora son privadas",
      featured_athlete_added: "Marcado como Atleta Destacado ⭐",
      featured_athlete_removed: "Eliminado de Destacados",
      user_not_found: "Usuario no encontrado",
      profile_copied: "¡Link del perfil copiado!",
      invite_copied: "¡Copiado!",
      invite_share_text: "¡Únete a Balancen! Usa mi código",
      search_foods: "Buscar alimentos",
      type_food_name: "Escribe un alimento...",
      search_and_add: "Buscar y agregar",
      selected_foods: "Alimentos seleccionados",
      total_nutrition: "Nutrición total",
      add_at_least_one_food: "Agrega al menos un alimento",
      save_failed: "Error al guardar. Intentá de nuevo.",
      meal_moment_title: "¿Qué estás comiendo ahora? 📸",
      meal_moment_subtitle: "Compártelo con tu equipo",
      meal_moment_scan: "Scan",
      create_your_profile: "Crea tu perfil",
      sign_in: "Iniciar sesión",
      sign_in_to_access_profile: "Inicia sesión para ver tu perfil",
      invite_description: "Tú y tu amigo ganan 5 días extra de Premium",
      joined_with_link: "se unió con tu link",
      share_failed: "Error al compartir",

      // Meal types and actions
      breakfast: "Desayuno",
      lunch: "Almuerzo",
      dinner: "Cena",
      snack: "Snack",
      add_meal: "Agregar comida",
      add_meal_photo: "Foto de comida",
      calories_today: "Calorías hoy",
      calories_goal: "Meta calórica",
      consumed: "Consumido",
      remaining: "Restante",
      completed: "Completado",
      complete_checkin: "Completar check-in",
      complete_day_label: "Completar día",
      analysis_complete: "Análisis completo",
      analyzing_food_step: "Analizando comida...",
      calculating_macros_step: "Calculando macros...",
      estimating_calories_step: "Estimando calorías...",
      detected_food: "Comida detectada",
      no_food_detected: "No se detectó comida",
      no_photo_captured: "No hay foto",
      no_meals_today: "Sin comidas hoy",
      meal_registered: "Comida registrada",
      meal_deleted: "Comida eliminada",
      error_saving_meal: "Error al guardar",
      error_deleting_meal: "Error al eliminar",
      error_processing_photo: "Error al procesar foto",
      retake_photo: "Retomar foto",
      retry_analysis: "Reintentar análisis",
      not_accurate_edit: "Editar si no es exacto",
      food_photo_optional: "Foto de comida (opcional)",
      estimated_calories: "Calorías estimadas",
      share: "Compartir",
      keep_going: "Sigue así",
      keep_flame_alive: "Mantén tu racha",
      on_fire: "¡Estás en racha!",
      great: "¡Genial!",
      goal_reached_perfect: "¡Meta alcanzada!",
      goal_almost_reached: "Casi en la meta",
      goal_exceeded: "Meta superada",
      goal_exceeded_back_on_track: "Superaste la meta, vuelve al camino",
      over_goal: "Sobre la meta",
      stay_under: "Mantente en la meta",
      stay_goal_label: "Objetivo",
      goal_text: "Meta",
      streak_milestone: "Hito de racha",
      mission_complete: "¡Misión completada!",
      just_completed_mission: "Completaste una misión",
      todays_mission: "Misión de hoy",
      todays_activity: "Actividad de hoy",
      one_task_left_midnight: "1 tarea antes de medianoche",
      tasks_left_midnight: "tareas antes de medianoche",
      tasks_remaining: "tarea restante",
      tasks_remaining_plural: "tareas restantes",
      complete_now_to_jump: "Completá ahora para subir",
      start_now_or_lose: "Empieza ahora o pierde tu racha",
      you_havent_completed: "No completaste",
      you_need_fire: "Necesitás fuego",
      fire: "Fuego",
      fire_to_pass: "fuego para pasar",
      gained_fire: "Ganaste fuego",
      more_for_free_month: "más para un mes gratis",
      invite_subtitle: "Invita amigos y gana beneficios",
      invite_reward_note: "+1 mes gratis por cada 3 amigos",
      invites_sent: "Invitaciones enviadas",
      more_meal: "comida más",
      more_meals: "comidas más",
      meals_logged: "Comidas registradas",
      first_meal_label: "Primera comida",
      tap_to_add_meal: "Toca para agregar comida",
      what_meal: "¿Qué comida?",
      how_was_today: "¿Cómo estuvo tu día?",
      how_did_you_eat: "¿Cómo comiste hoy?",
      did_you_move: "¿Te moviste hoy?",
      add_daily_note: "Agregar nota del día",
      weight_today_optional: "Peso hoy (opcional)",
      current_weight_text: "Peso actual",
      checkin_completed: "Check-in completado",
      live_activity: "Actividad en vivo",
      open_app: "Abrir app",
      expires_24h: "Vence en 24h",
      add_status: "Agregar estado",
      save_status: "Guardar estado",
      status_max_60_chars: "Máx 60 caracteres",
      error_updating_status: "Error al actualizar estado",
      copied: "Copiado",
      select_at_least_one: "Seleccioná al menos uno",
      premium_only: "Solo Premium",
      subscribed: "Suscrito",
      referrals: "Referencias",
      group_leaderboard: "Leaderboard del grupo",
      since_start_text: "Desde el inicio",
      vs_yesterday_text: "vs ayer",
      this_week_text: "Esta semana",
      today_calories: "Calorías de hoy",
      not_logged_today: "No registrado hoy",
      building_consistency: "Construyendo consistencia",
      by_logging: "Al registrar",
      ai_health_coach: "Coach de salud IA",
      advanced_analytics: "Análisis avanzado",
      personalized_health_insights: "Insights personalizados",
      smart_challenge_suggestions: "Sugerencias de retos",
      weekly_ai_summaries: "Resúmenes semanales IA",
      unlock_personalized_insights: "Desbloquear insights",
      upgrade_to_premium_cta: "Actualizar a Premium",
      youre_leading: "¡Estás liderando!",
      someone: "Alguien",
      kcal_added: "kcal agregadas",
      invite_message_template: "¡Únete a Balancen! Usa mi código {code} para un trial Premium gratis. https://balancen.app",
      poor: "Malo",
      capture: "Capturar",
      keep_it_up: "Sigue así",
      canvas: "lienzo",
      }
      }
      };

import { getLocalLanguage } from "@/lib/language";

const getInitialLanguage = () => getLocalLanguage() || "en";

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: getInitialLanguage(),
    fallbackLng: "en",
    defaultNS: "translation",
    interpolation: {
      escapeValue: false
    },
    react: {
      useSuspense: false
    }
  });

// Language persistence is handled centrally via lib/language.js setLocalLanguage()
// No listener needed here.

export default i18n;