...
export default function JoinByCodeModal({ open, onClose, user, onJoined }) {
  ...
  const handleJoin = async () => {
    ...
    if (!group.allow_join_by_code) { toast.error("This group doesn't allow joining by code. Ask the admin to invite you by email."); setLoading(false); return; }
    const existing = await base44.entities.GroupMember.filter({ group_id: group.id, user_email: user.email });
    if (existing.length) { toast.info("You're already in this group"); setLoading(false); onJoined(); onClose(); return; }
